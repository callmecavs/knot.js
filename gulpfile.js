...
const attribution =
`/*!
 * Knot.js ${ json.version } - ${ json.description }
...
gulp.task('js', () => {
  return rollup
    ...
    .then(bundle => {
      ...
      fs.writeFileSync('dist/maps/knot.js.map', files.map.toString())
    })
})
...
gulp.task('watch', () => {
  gulp.watch('src/knot.js', ['js', reload])
})
